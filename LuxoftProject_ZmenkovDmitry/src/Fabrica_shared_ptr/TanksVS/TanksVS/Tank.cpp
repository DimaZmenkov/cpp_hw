 ...
 Rect Tank::getRect() const
{
 ...
 Point Tank::getCoord() const
 {
  ...
  void Tank::shoot(Rect& rect, bool& isMain, Direction &direction)
 {
	 //Rect rect;
	 rect.left = coord_.x; rect.up = coord_.y; rect.bottom = rect.up + 5;
	 rect.right = rect.left + 5;
	 direction = isMain_ ? direction_ :rightMove;
	 isMain = isMain_;
	
	 /*bullet = bf -> createMilitaryInstance(rect, isMain_, direction);
	 bullet->setRect(rect);
	 bullet->setIsMain(isMain_);
